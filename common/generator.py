...
class ChunkedGenerator:
    ...
    def get_batch(self, seq_i, start_3d, end_3d, flip, reverse):
        ...
        if flip:
            ...
            self.batch_2d_GT[:, self.kps_left + self.kps_right] = self.batch_2d_GT[:, self.kps_right + self.kps_left]
        if reverse:
        ...
        if self.poses_3d is None and self.cameras is None:
            ...
            return np.zeros(
                9), self.batch_3d.copy(), self.batch_2d.copy(), self.batch_2d_GT.copy(), action, subject, int(cam_index)
        elif self.poses_3d is None:
            return self.batch_cam, None, self.batch_2d.copy(), self.batch_2d_GT.copy(), action, subject, int(cam_index)
        else:
            return self.batch_cam, self.batch_3d.copy(), self.batch_2d.copy(), self.batch_2d_GT.copy(), action, subject, int(
                cam_index)
