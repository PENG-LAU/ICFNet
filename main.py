...
from model.trans import ICFNet
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader)):
        ...
        out_target[:, :, 0] = 0
        if split == 'train':
            w_mpjpe = torch.tensor([1, 1, 2.5, 2.5, 1, 2.5, 2.5, 1, 1, 1, 1, 1, 2, 2, 1, 2, 2]).cuda()  # this is base
...
if __name__ == '__main__':
    print('cuda is_available : "{0}"'.format(torch.cuda.is_available()))
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.317, patience=10, verbose=True)
